...
Committed `de404ca`. Then committed the two stale `reports/2026-09-07-*` tweaks as `ea3b1aa` and **pushed everything** — `master` == `origin/master`, tree clean. (`modules/gps.js` was already committed `72f2737` in the 9/7 session; not in the tree.)

**Pi live check — "can we add an item to DoorDash on the Pi": YES.** After a full Pi reboot (browser was in the documented degraded state) + clean prewarm: `GET /api/food/search?q=tacos` (fresh, 8 restaurants) → `POST /api/food/select` Taco Time `29280516` (39 menu items, all clean names + real ids — Day 3's `cleanScrapedName`/SSR parser holding) → `POST /api/food/cart/add` itemIndex 2 "Mexi-Fries®" returned `needsOptions` ("Choose Your Size") → re-sent with `selections:[{groupIndex:0,optionIndex:0,optionText:"Small"}]` → **`[API] ✅ Item added via HTTP API` — `addCartItem: ok=true status=200`** (real DoorDash cart mutation via the Day-2 HTTP fast path).

**Still broken (Day 5 territory, plus environmental):** `readBrowserCart()` and `clearBrowserCart()` — their `page.evaluate()` calls against DoorDash pages time out (8–10s) *even on a cold browser right after a full Pi reboot + successful prewarm*. Not transient load — `page.evaluate` on the DoorDash `/home` page is hanging outright (heavy React page, Pi 4 main thread). Consequence: **no real-cart ground-truth read is possible right now** — every `my cart` response is echoing local SQLite state (`restaurants.formatCart`), not a browser scrape. So the test Mexi-Fries (and possibly the older stray "Chips & Salsa") may still sit in the real DoorDash browser cart. **Zero charge risk** — `DOORDASH_DRY_RUN=true`, checkout never called, and the next real `/api/food/select` runs `clearBrowserCart()` + `db.clearCart()`. Local SQLite cart cleared to empty at session end; both services `active`.

This also means **Day 1's checkout-line-item scrape can't be verified on the Pi at all** (same `page.evaluate` hang would hit `scrapeCheckoutLineItems()`), reinforcing that its real check belongs on Railway (Day 4).
...
## Session 2026-09-11 — week-of-Sep-8 plan Day 2: multi-restaurant smoke test + triage. No code changes, findings only.

Fresh Pi reboot at session start (uptime ~2min), full prewarm, then drove search → select → add → verify (`my cart` voice phrase, `actions`-marked reads only) across 5 restaurants via the touchscreen HTTP routes (`/api/food/search`, `/api/food/select`, `/api/food/cart/add`), no checkout. Requests against the Pi's browser routinely take 25–90s (search ~40–55s, select ~50–60s, add ~25–50s) — normal for this hardware, not a regression.

**Wingstop** — search/select/menu clean (Day 3's `cleanScrapedName`/SSR id-parser still holding, zero garbled names in 38 items). **New finding:** both **Cajun Fried Corn** and **Veggie Sticks** hit the same bug class already known for Costa Vida's "Single Taco" (2026-09-01 notes) — `itemPage`'s `optionLists` (the HTTP fast-path's source of truth) reports only ONE required group ("Choose Size"), the user's selection for it is accepted, then `addCartItemV2` rejects with `item_validation_error` for a SECOND required group `itemPage` never surfaced at all ("Corn Seasoning Options" / "Modify Veggie Sticks" respectively). The Playwright DOM fallback that runs next independently corroborates the miss — its own "Extracting REQUIRED options only" pass also finds just the 1 group. So this isn't an `itemPage`-only gap; DoorDash's DOM doesn't expose the second requirement structurally either, at least not to the current extraction logic. Both attempts failed clean (`needsOptions` back to the caller, ~35-50s, no hang, no wrong/partial add, real cart untouched) — **not a data-integrity risk, but likely affects most/all Wingstop items with a "seasoning"/"modify" sub-group**, not just the 2 tested. Worth a fix pass: detect `item_validation_error`'s named group from the API failure and re-surface it as an additional required group (same fix already suggested for Costa Vida's Single Taco on 2026-09-01, now confirmed as the right general fix, not a one-off).

**Costa Vida** — Chips & Salsa (Small) added via the DOM path (synthetic `item-12` id — a size-grouped item, no real DoorDash item id), verified in the real cart via `my cart` (`1x Chips & Salsa - $4.49`, matching 2026-09-07's result). Single Taco not re-tested (already documented broken, same bug class as above).

**Little Caesars** — Crazy Bread (no options) added cleanly and fast-pathed, verified in the real cart. Custom Stuffed Crust (the 2026-09-01 Day-2 fix subject) not re-tested this session — already verified working then.

**Taco Time** — Crisp Taco + Beef added cleanly via the HTTP fast path (real `_apiGroupId`/`_apiOptions`, single required group, no hidden second group this time), verified in the real cart. Minor, unconfirmed-as-a-bug observation: the local cart record's `selectedOptions` came back empty for this fast-path add (DOM-path adds, e.g. Costa Vida's Pico De Gallo, do carry `selectedOptions`) — cosmetic only, the `my cart` text format doesn't surface `selectedOptions` either way so it wasn't user-visible here; worth a look if `[REORDER]` or the kiosk's own cart display ever need it.

**Cafe Rio (the 5th/new restaurant)** — previously only touched via a failed SMS-path scrape attempt (2026-09-09, `extractMenuItems` timeout) that was never actually completed. This session: search/select/menu came back clean (23 real items, no garbage) on the touchscreen path. Added "Chips" — the add legitimately succeeded (confirmed present in the real cart afterward) but the full request (item search + add + post-add price-verify readBrowserCart) ran long enough (>90s) that my test client's own curl timeout gave up waiting before the server responded; the server kept working and finished on its own. Reading the result back reproduced the **known Day-5 `readBrowserCart()` name-mash bug** fresh, with a new example: `"1x Chips$6.382 ×2 ×2 × - $0.00"` (Subtotal/Tax $0.00, Total only the delivery fee) — same shape as the 2026-09-07 "Chips & Salsa" mash, different restaurant, same underlying cart-row selector mismatch. Reinforces that Day 5 (the `readBrowserCart()` DOM fix) needs a fresh cart-row DOM dump, not another guess.

**One unplanned server auto-restart** occurred mid-session (~13:18, no error logged, systemd just relaunched the unit) — matches this file's long-documented "Xvfb/Chromium fragility under sustained load" pattern. Recovered on its own with a clean prewarm; no manual `pkill` needed this time, unlike several past sessions.

**Session end:** real cart cleared via `/api/food/cart/clear` and reconfirmed genuinely empty via a `my cart` / `actions`-marked voice read (not trusting the clear endpoint's immediate response, per this file's standing lesson — `clearBrowserCart()` fires unawaited). `DOORDASH_DRY_RUN=true` throughout, zero charge risk. No code changed this session — findings only, per the plan's "the point is a written triage, not fixes."
