...
## Session 2026-08-25 continued (Day 2) — root-caused and fixed the multi-minute stepper-item hang from earlier today. Committed `<pending>`, not yet pushed.
...
1. Confirm with the user whether to commit + push this fix (holding per repo's git safety rules — not committed automatically).
2. Re-test the stepper-hang repro a few more times over a longer session to build more confidence it's genuinely fixed and not just non-deterministically avoided today (this codebase's own history shows real flakiness in the Xvfb/Chromium stack under load).
3. Consider verifying the "modal already open" success-without-verification gap doesn't produce a false positive on some other item/restaurant.
4. Everything else from the standing backlog unchanged: GPS wiring, review-text-as-menu-item scraping (Wingstop/Costa Vida), physical-tap 18px-threshold confirmation, food.html multi-select stepper UI, the `extractRestaurantList` 15s timeout noticed 2026-08-25 morning, multi-unit stepper design task.
